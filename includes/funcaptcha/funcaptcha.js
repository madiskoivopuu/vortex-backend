...
class FunCaptcha {
	constructor(scope) {
		...
		this.user_agent_string = this.user_agent.data.userAgent;
	...
	async InitCaptcha(proxy) {
		...
		return {
			...
			solved: gcParams["sup"] === "1" ? true : false
		};
	}
	...
	async GetGameData() {
		...
		this.rotations = Math.round(360/this.angle);
		...
		// get all images and store them in cachedImageData
		this.gfct.game_data.customGUI._challenge_imgs.forEach(imageUrl => {
			var promise = this.requests.get({
				url: imageUrl,
				timeout: 25000,
				headers: headers,
			}).catch(e => {});
			this.cachedImageData.push(promise);
		})
	...
	async DecryptImage() {
		if(this.currentWave-1 >= this.gfct.game_data.customGUI._challenge_imgs.length) {
			return {
				status: "fail",
		...
		try {
			var response = JSON.parse(await this.cachedImageData[this.currentWave-1]);
		} catch(error) {
			return {
				status: "fail",
